  ...
  return <AdminPage>
    ...
    <Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-h-[92vh] max-w-6xl overflow-y-auto"><DialogHeader><DialogTitle>{t(editing ? "editTitle" : "createTitle")}</DialogTitle><DialogDescription>{t("canvasDescription")}</DialogDescription></DialogHeader><div className="grid gap-4"><div className="grid gap-3 md:grid-cols-2"><div><Label>{t("key")}</Label><Input disabled={!!editing} value={form.key} onChange={(event) => setForm({ ...form, key: event.target.value })} placeholder="storyboard_generation" /></div><div><Label>{t("name")}</Label><Input value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder={t("namePlaceholder")} /></div></div><div><Label>{t("workflowDescription")}</Label><Textarea value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} placeholder={t("descriptionPlaceholder")} /></div>
      <div className="rounded-xl border border-border bg-[#fbfbfa] p-3"><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><div><p className="text-sm font-semibold">{t("canvasTitle")}</p><p className="text-xs text-muted-foreground">{t("canvasHelp")}</p></div><Badge variant={workflowReady ? "secondary" : "outline"}>{workflowReady ? t("pathReady") : t("pathNeedsAttention")}</Badge></div><div className="grid gap-3 lg:grid-cols-[190px_minmax(0,1fr)_260px]"><aside className="rounded-lg border border-dashed border-border bg-background p-2"><p className="px-2 pb-2 text-[11px] font-medium uppercase tracking-[0.12em] text-muted-foreground">{t("nodeLibrary")}</p><div className="space-y-1">{availableNodes.length === 0 ? <p className="px-2 py-3 text-xs text-muted-foreground">{t("allNodesAdded")}</p> : availableNodes.map((node) => { const Icon = nodeIcon(node.icon); return <button key={node.id} draggable onDragStart={(event) => { event.dataTransfer.setData("application/x-workflow-activity", node.activity); setDraggedNodeID(node.id) }} onClick={() => appendNode(node)} className="flex w-full items-start gap-2 rounded-md px-2 py-2 text-left text-xs transition-colors hover:bg-secondary"><Icon className="mt-0.5 h-3.5 w-3.5 text-primary" /><span><span className="block font-medium text-foreground">{t(node.titleKey)}</span><span className="block pt-0.5 leading-4 text-muted-foreground">{t(node.descriptionKey)}</span></span></button> })}</div></aside>
        ...
        <div className="mt-4 overflow-hidden rounded-lg border border-border bg-background"><div className="border-b border-border px-3 py-2"><p className="text-sm font-semibold">{t("stepTableTitle")}</p><p className="mt-0.5 text-xs text-muted-foreground">{t("stepTableHelp")}</p></div><div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="bg-muted/40 text-xs text-muted-foreground"><tr><th className="w-16 px-3 py-2 font-medium">{t("stepOrder")}</th><th className="px-3 py-2 font-medium">{t("stepName")}</th><th className="px-3 py-2 font-medium">{t("stepAction")}</th><th className="px-3 py-2 font-medium">{t("stepDependency")}</th><th className="min-w-72 px-3 py-2 font-medium">{t("stepInstruction")}</th></tr></thead><tbody>{form.nodes.map((node, index) => <tr key={node.id} onClick={() => setSelectedNodeID(node.id)} className={`cursor-pointer border-t border-border align-top transition-colors hover:bg-muted/30 ${selectedNodeID === node.id ? "bg-primary/5" : ""}`}><td className="px-3 py-3 font-medium text-muted-foreground">{index + 1}</td><td className="px-3 py-3 font-medium">{t(node.titleKey)}{node.required && <span className="ml-2 text-xs font-normal text-primary">{t("required")}</span>}</td><td className="px-3 py-3 text-xs text-muted-foreground">{t(node.descriptionKey)}</td><td className="px-3 py-3 text-xs text-muted-foreground">{index === 0 ? t("noDependency") : t(form.nodes[index - 1].titleKey)}</td><td className="px-3 py-2"><Textarea aria-label={`${t(node.titleKey)} ${t("nodeNote")}`} className="min-h-20 resize-y bg-background text-sm" value={node.note} onClick={(event) => event.stopPropagation()} onChange={(event) => updateNode(node.id, { note: event.target.value })} placeholder={t("nodeNotePlaceholder")} /></td></tr>)}</tbody></table></div></div>
        <div className="mt-4 overflow-hidden rounded-lg border border-border bg-background"><div className="border-b border-border px-3 py-2"><p className="text-sm font-semibold">{t("settingsTableTitle")}</p><p className="mt-0.5 text-xs text-muted-foreground">{t("settingsTableHelp")}</p></div><div className="grid divide-y divide-border sm:grid-cols-3 sm:divide-x sm:divide-y-0"><div className="p-3"><Label htmlFor="workflow-duration" className="text-xs">{t("maxDuration")}</Label><Input id="workflow-duration" className="mt-2" type="number" min="1" max="12" step="0.5" value={form.settings.maxDurationHours} onChange={(event) => setForm((current) => ({ ...current, settings: { ...current.settings, maxDurationHours: event.target.value } }))} /><p className="mt-1 text-xs text-muted-foreground">{t("maxDurationHelp")}</p></div><div className="p-3"><Label htmlFor="workflow-parallelism" className="text-xs">{t("maxParallelism")}</Label><Input id="workflow-parallelism" className="mt-2" type="number" min="1" max="32" step="1" value={form.settings.maxParallelism} onChange={(event) => setForm((current) => ({ ...current, settings: { ...current.settings, maxParallelism: event.target.value } }))} /><p className="mt-1 text-xs text-muted-foreground">{t("maxParallelismHelp")}</p></div><div className="p-3"><Label htmlFor="workflow-attempts" className="text-xs">{t("maxAttempts")}</Label><Input id="workflow-attempts" className="mt-2" type="number" min="1" max="10" step="1" value={form.settings.maxAttempts} onChange={(event) => setForm((current) => ({ ...current, settings: { ...current.settings, maxAttempts: event.target.value } }))} /><p className="mt-1 text-xs text-muted-foreground">{t("maxAttemptsHelp")}</p></div></div></div></div>
